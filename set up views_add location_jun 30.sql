...
-- 创建包含地理信息的问题热点分析视图
CREATE VIEW problem_hotspot_analysis AS
SELECT 
    m.delivery_lat,
    m.delivery_lng,
    m.restaurant_lat,
    ...
    -- 计算问题指标
    CASE WHEN m.delivery_time > 45 THEN 1 ELSE 0 END as is_overtime,
    CASE WHEN m.rider_rating < 3.0 THEN 1 ELSE 0 END as is_low_rating,
    CASE WHEN m.delivery_time > 60 THEN 1 ELSE 0 END as is_severe_delay,
    
    -- 综合问题得分
    (CASE WHEN m.delivery_time > 45 THEN 2 ELSE 0 END) +
    (CASE WHEN m.rider_rating < 3.0 THEN 2 ELSE 0 END) +
    (CASE WHEN m.delivery_time > 60 THEN 1 ELSE 0 END) as problem_score
    
FROM mysql_ready m
WHERE m.delivery_lat IS NOT NULL 
  AND m.delivery_lng IS NOT NULL;
